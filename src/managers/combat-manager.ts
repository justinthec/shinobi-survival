    ...
    static handleMovement(game: ShinobiClashGame, p: PlayerState, input: DefaultInput) {
        if (p.dash.active) {
            ...
            game.particles.push({
                ...
                vel: new Vec2((rand() - 0.5) * 2, (rand() - 0.5) * 2),
                life: 15,
                maxLife: 15,
    ...
    static tryDash(game: ShinobiClashGame, p: PlayerState) {
        ...
        for(let i=0; i<5; i++) {
             game.particles.push({
                ...
                type: 'smoke',
                pos: new Vec2(p.pos.x, p.pos.y),
                vel: new Vec2((rand(i)-0.5)*5, (rand(i+10)-0.5)*5),
                life: 20, maxLife: 20, color: 'white', size: 6
            });
        }
    }
        ...
        for (let i = game.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.type === 'clone_strike') {
                ...
                if (nearest) {
                    ...
                    const speed = 7; // Slightly faster than players
