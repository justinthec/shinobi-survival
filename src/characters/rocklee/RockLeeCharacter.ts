...
export class RockLeeCharacter implements CharacterDefinition {
    ...
    render(ctx: CanvasRenderingContext2D, state: PlayerState, time: number, isLocal: boolean, isOffCooldown: boolean, showHealthBar: boolean = true) {
        ...
        if (isQActive) {
            ...
            const totalSpins = 5;
            // "Quadratic that doesn't ease out but has fast start": V(t) > 0 at t=1.
            // Curve: p * (1.5 - 0.5 * p). Velocity: 1.5 - p. Starts at 1.5x, ends at 0.5x speed.
            const curve = progress * (1.5 - 0.5 * progress);
